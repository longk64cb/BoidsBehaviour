...
public class ObstacleAvoidanceBehaviour : SteeringBehaviour
{
    ...
    public override (float[] danger, float[] interest) GetSteering(float[] danger, float[] interest, AIData aiData)
    {
        foreach (Collider2D obstacleCollider in aiData.obstacles)
        {
            ...
            for (int i = 0; i < Directions.detectDirections.Count; i++)
            {
        ...
        if (Application.isPlaying && dangersResultTemp != null)
        {
            if (dangersResultTemp != null)
            {
                ...
                for (int i = 0; i < dangersResultTemp.Length; i++)
                {
                    Gizmos.DrawRay(
                        transform.position,
                        Directions.detectDirections[i] * dangersResultTemp[i]*2
                        );
                }
            }
        }
...
public static class Directions
{
    ...
    //        new Vector2(-1,1).normalized
    //    };
    public static List<Vector2> detectDirections = new List<Vector2>();
    public static int GetDirectionCount => detectDirections.Count;
}
